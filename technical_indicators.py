...
class TechnicalIndicators:
    ...
    def __init__(self, config):
        ...
        # Use config or default important indicators
        # Setup logging
    ...
    def calculate_indicators(self, ticker, interval):
        ...
        # Find the correct OHLCV file for this ticker/interval
        ...
        # Logging: use logging for all info and error messages
        ...
        if df.empty or not set(['open', 'high', 'low', 'close', 'volume']).issubset(df.columns):
            logging.warning(f"OHLCV data for {ticker} ({interval}) is empty or missing columns, skipping.")
            return
        # Ensure all price/volume columns are numeric
        for col in ['open', 'high', 'low', 'close', 'volume']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
        # Ensure DataFrame is sorted by date and has no duplicate indices
        ...
        if len(df) < 20:
            logging.warning(f"Not enough data to calculate indicators for {ticker} ({interval}), skipping.")
            return
        ...
        try:
            ...
            # Output columns: KCLe, KCUe, KCB (from pandas_ta.kc)
            if kc is not None and isinstance(kc, pd.DataFrame):
                kc_cols = kc.columns
                df['KCLe'] = kc['KCLe_20_2.0'].map(to8)
                df['KCBe'] = kc['KCBe_20_2.0'].map(to8)
                df['KCUe'] = kc['KCUe_20_2.0'].map(to8)
            else:
                df['KCLe'] = df['KCUe'] = df['KCB'] = np.nan
        except Exception as e:
        ...
        try:
            ...
            if donchian is not None and isinstance(donchian, pd.DataFrame):
                donchian_cols = donchian.columns
                df['DCL'] = donchian['DCL_20_20'].map(to8)
                df['DCM'] = donchian['DCM_20_20'].map(to8)
                df['DCU'] = donchian['DCU_20_20'].map(to8)
            else:
        ...
        try:
            ...
            df[[f'SR_High_{i+1}' for i in range(N)]] = np.nan
            df[[f'SR_Low_{i+1}' for i in range(N)]] = np.nan

        # --- Chart Pattern Detection (enhanced) ---
        df['chart_pattern'] = ''
        from scipy.signal import argrelextrema
        close = df['close']
        ...
        for i in range(1, len(local_min)):
            if abs(close.iloc[local_min[i]] - close.iloc[local_min[i-1]])/close.iloc[local_min[i-1]] < 0.05:
        ...
        for i in range(1, len(local_max)):
            if abs(close.iloc[local_max[i]] - close.iloc[local_max[i-1]])/close.iloc[local_max[i-1]] < 0.05:
        ...
        for i in range(window, len(df)):
            ...
            if len(peaks) >= 3:
                for j in range(len(peaks)-2):
                    ...
                    if w.iloc[m] > w.iloc[l] and w.iloc[m] > w.iloc[r] and abs(w.iloc[l] - w.iloc[r])/w.iloc[m] < 0.15:
        ...
        for i in range(window, len(df)):
            w = close.iloc[i-window:i]
            if w.pct_change().abs().max() > 0.1 and w[-5:].std() < w.std()/2:
                df.at[w.index[-1], 'chart_pattern'] = 'Flag/Pennant'
        # Cup and Handle (U shape followed by small dip)
        for i in range(window, len(df)-5):
            w = close.iloc[i-window:i]
            if w.min() == w.iloc[window//2] and w.iloc[-1] > w.iloc[window//2] and w.iloc[-5:].min() < w.iloc[-10:-5].min():
        ...
        try:
            ...
            for i in range(2, len(df)):
                ...
                # Morning Star
                elif (ppo > ppc and abs(ppo-ppc)/abs(pph-ppl) > 0.5 and
                ...
                # Spinning Top
                elif range_ > 0 and 0.1 < body/range_ < 0.3:
                    df.at[df.index[i], 'candle_pattern'] = 'Spinning Top'
        except Exception as e:  
            logging.error(f"Error calculating Candle Pattern: {e}")
            df['candle_pattern'] = np.nan     


        # Multi-timeframe merge (1h + 1d)
        if interval == '1h':
            daily_file = [f for f in os.listdir(self.indicator_dir) if f.startswith(f"{ticker}_indicators_1d")]
            if daily_file:
                df_daily = pd.read_csv(os.path.join(self.indicator_dir, daily_file[-1]))
                if 'date' in df_daily.columns:
                    df_daily['date'] = pd.to_datetime(df_daily['date'])
                    daily_merge_cols = [col for col in df_daily.columns if col != 'date']
                    rename_map = {col: f"{col}_1d" for col in daily_merge_cols}
                    df_daily = df_daily[['date'] + daily_merge_cols].rename(columns=rename_map)
                    df = df.reset_index()
                    df = pd.merge_asof(df.sort_values('date'), df_daily.sort_values('date'), on='date', direction='backward')
                    df.set_index('date', inplace=True)
        ...
        # --- Output ---
        # Remove rows where any important indicator is missing
        present_indicators = [col for col in df.columns if col in df.columns]
        if present_indicators:
            df.dropna(subset=present_indicators, inplace=True)
